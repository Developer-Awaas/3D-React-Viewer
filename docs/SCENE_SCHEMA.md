...
## Fields
    ...
    openings[]{ id, type:"window"|"door", wall:<wall id>,
                along:[a,b]    position along that wall, in feet
...
A wall is tiled into solid sub-boxes that skip the opening void:
...
## Where CubiCasa fits
CubiCasa detects wall pixels -> `walls.py` vectorizes to centre-lines ->
Step B converts centre-line + `wall_types` thickness into these wall footprints
(and, later, openings from the door/window detections).
