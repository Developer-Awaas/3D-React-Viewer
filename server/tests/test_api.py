...
def test_scene_glb_streams_bytes_without_temp_files(client, monkeypatch):
    """Regression: a fixed temp filename let concurrent users download each
    other's building (and errors leaked temp files). The endpoint now exports
    straight to bytes: each call must return a valid GLB body and leave no
    drishti_ temp files behind."""
    ...
    monkeypatch.setattr(pdf_vector, "parse",
                        lambda raw, w, wing="largest": marker)
    pattern = os.path.join(tempfile.gettempdir(), "drishti_*.glb")
